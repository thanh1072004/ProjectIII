...
\label{sec:dvd}
The rapid development of the Internet and web applications has brought immense benefits, transforming how organizations and individuals interact and conduct transactions globally. However, the increasing reliance on web applications has also introduced critical security risks, as the web layer has become one of the most frequently targeted attack surfaces. Imperva's Web Application Attack Report, for instance, found that more than three quarters of the monitored web applications were attacked by every one of the major attack categories it tracked~\cite{imperva}, underscoring how systematically web applications are probed by adversaries. Common attack vectors such as injection flaws (e.g., SQL or OS Injection), Cross-Site Scripting (XSS), Broken Access Control, and Cross-Site Request Forgery (CSRF)~\cite{owasp2021} are continuously exploited by attackers to steal sensitive information, seize control of systems, and disrupt services. The resulting consequences include not only severe financial losses but also significant and long-lasting damage to corporate reputation.
...
\item The thesis designs and implements a Smart Consensus mechanism based on risk compensation across tiers. Practical testing (Chapter 4) proves this mechanism reduces the false positive rate by 38\% compared to relying solely on standalone unsupervised models.
\item Regarding empirical performance, the thesis evaluated the system on a large-scale, balanced dataset comprising 111,065 logs (combining the CSIC 2010 database~\cite{csic2010} with synthetic traffic), with the full evaluation reported in Chapter 4. The proposed solution achieved outstanding results with an F1-score of 94.30\%, a recall of 97.62\%, and an IDS standard F2-score of 96.26\%, demonstrating its readiness for integration into real-world network monitoring environments.
\end{enumerate}
