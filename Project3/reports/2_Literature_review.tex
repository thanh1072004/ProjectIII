...
$91.54\%$. Of particular relevance to this thesis is the work of Vartouni et
al.~\cite{vartouni2018}, which is the closest prior study to the unsupervised
tier proposed here and therefore deserves a fuller treatment. Instead of
training a classifier on labelled attacks, they cast web-attack detection as
an \emph{anomaly detection} problem. A Stacked Auto-Encoder---a neural network
trained to compress its input into a low-dimensional code and then reconstruct
it---is fitted on \emph{normal} requests only, so that it learns a compact
model of what legitimate traffic looks like; the learned representation is then
passed to an isolation-based outlier detector, and any request that lies far
from the learned normal manifold (equivalently, that the network reconstructs
poorly) is flagged as anomalous. Because the model never observes an attack
during training, it follows exactly the \emph{one-class learning} principle
adopted by the third tier of this thesis and can, in principle, detect
previously unseen attacks without any attack labels. On CSIC 2010 the method
attains an accuracy of about $88.32\%$. This result is doubly informative for
the present work: it confirms that the unsupervised, label-free direction is
viable on the very same dataset, but it also exposes the characteristic
weakness of a \emph{purely} unsupervised detector. Its verdict rests on an
anomaly-score threshold, so precision and recall trade off sharply against the
choice of that threshold, and it possesses neither the calibrated,
label-grounded confidence of a supervised classifier nor the pinpoint
precision of an explicit signature. This limitation is the direct motivation
for letting a supervised model validate the anomaly tier in the present
design. Betarte et al.~\cite{betarte2018} take a complementary, supervised
view, comparing Random Forest, K-Nearest Neighbours, and Support Vector
Machine, with Random Forest performing best in that group. Taken together,
these works reveal two trends: on the one hand, complex deep models do not
necessarily outperform compact structural classifiers on CSIC 2010; on the
other hand, the reconstruction-based anomaly direction opens up the
possibility of detecting novel attacks without requiring attack labels during
training---at the cost of the precision and stability that a signature or a
supervised model would provide.
...
algorithms. Crucially, even the studies that move beyond supervised
learning---most notably the Stacked Auto-Encoder of Vartouni et
al.~\cite{vartouni2018}, which is itself a single anomaly detector---employ
one paradigm in isolation: none of the surveyed systems unites
signature-based, supervised, and unsupervised detection within a single
decision pipeline. This three-way combination, rather than the choice of any
individual algorithm, is the precise gap that this thesis sets out to fill.
Second, most studies work on offline pre-processed feature
...
This chapter has established the foundation for the design that follows.
Section~\ref{sec:scope} delimited the problem---detecting application-layer
web attacks from Apache logs---with a catalogue of in-scope attack classes and
the excluded limitations. Section~\ref{sec:related} surveyed representative
studies on CSIC 2010 and showed that most pursue a single approach and rarely
combine signature-based detection with machine learning; this gap motivates
the three-tier hybrid architecture of the thesis.
Sections~\ref{sec:ids} through~\ref{sec:features} then presented the
supporting knowledge: the notion of an IDS and its two paradigms, OWASP web
attack types, the signature technique with its anti-evasion decoding, the
supervised (Random Forest, Logistic Regression) and unsupervised one-class
(LOF, Isolation Forest, One-Class SVM) algorithms, and the shared
$22$-feature extraction with F2-centred metrics. The next chapter builds on
this to present the detailed three-tier design and the smart consensus
decision mechanism.

% =====================================================================
% BIBLIOGRAPHY
% Move these \bibitem entries into your global .bib / thebibliography and
% renumber to stay consistent with the rest of the thesis.
% =====================================================================
\end{document}