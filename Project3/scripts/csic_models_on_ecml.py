#!/usr/bin/env python3
"""
CROSS-DATASET test: take the MAIN system's CSIC-trained models (the deployed
trained_models/*_final.pkl) and evaluate them, WITHOUT any retraining, on the
harder ECML/PKDD 2007 test set. This measures how well the system generalises
to data it has never seen.

It reuses the EXACT prediction logic of scripts/evaluate_real_hybrid.py
(RF/LR/IF on raw features, OCSVM/LOF on scaler_final-transformed features,
plus the production regex tier and the Smart Consensus / Simple Voting
fusions), so the CSIC numbers it would produce match the main report.

Read-only on all models. Writes only into datasets/ecml/charts/ with a
"csic_on_ecml_" prefix, so it touches neither the CSIC artifacts nor the
"trained-on-ECML" experiment outputs.
"""
...
import apache_log
apache_log.PATH_PARAM_VOCAB = {}          # final CSIC models trained with empty vocab

MODELS_DIR = os.path.join(PROJECT_ROOT, "trained_models")           # CSIC models (read-only)
EVAL_FILE  = os.path.join(PROJECT_ROOT, "datasets", "ecml", "ecml_test.log")
CHARTS_DIR = os.path.join(PROJECT_ROOT, "datasets", "ecml", "charts")
...
def main():
    ...
    with open(EVAL_FILE, 'r', errors='ignore') as f:
        for line in f:
            ...
            p = urlparse(decoded)
            try:
                feats = _detector.extract_features(p.path or "/", p.query or "",
                                                   method, status, ua, referer)
            except Exception:
                continue
            X.append(feats)
    ...
    print(f"  ECML test = {n} samples ({n_atk} attack, {n_cln} clean, {n_atk/n*100:.2f}% attack)")

    # ---------- load CSIC-trained models (read-only) ----------
    ...
    # ---------- predictions (mirror evaluate_real_hybrid.py exactly) ----------
    ...
    print("\n=== CSIC-trained models tested on ECML/PKDD 2007 (no retraining) ===")
    print(f"  {'Configuration':32} {'Prec':>7} {'Recall':>7} {'F1':>7} {'F2':>7}")
    for name, mtr in results.items():
        print(f"  {name:32} {mtr['precision']:7.2f} {mtr['recall']:7.2f} {mtr['f1']:7.2f} {mtr['f2']:7.2f}")

    # ---------- save json ----------
    out = {
        "timestamp": datetime.now().isoformat(),
        "experiment": "CSIC-trained models -> ECML/PKDD 2007 test (cross-dataset, no retraining)",
        "models_source": "trained_models/*_final.pkl (CSIC)",
        "evaluation": {"file": "datasets/ecml/ecml_test.log", "samples": n,
                       "attacks": n_atk, "clean": n_cln,
                       "attack_ratio": round(n_atk / n * 100, 2)},
        "results": [{"model": k, **v} for k, v in results.items()],
    }
    with open(os.path.join(CHARTS_DIR, "csic_on_ecml_results.json"), "w", encoding="utf-8") as f:
        json.dump(out, f, indent=2)
    print(f"\n[json] saved csic_on_ecml_results.json to {CHARTS_DIR}")

    # ---------- chart: grouped metric bars ----------
    names = list(results.keys())
    P  = [results[k]["precision"] for k in names]
    R  = [results[k]["recall"] for k in names]
    F1 = [results[k]["f1"] for k in names]
    F2 = [results[k]["f2"] for k in names]
    ...
    ax.set_title("CSIC-trained models tested on ECML/PKDD 2007 (no retraining)", fontweight="bold")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(CHARTS_DIR, "csic_on_ecml_metrics.png"), dpi=150, bbox_inches="tight")
    plt.close()
    ...
    for i, (name, pred) in enumerate(panels):
        cm = confusion_matrix(y, pred, labels=[0, 1]); mtr = results[name]
        ...
        ax.set_title(f"{name}\nF1={mtr['f1']:.1f}  F2={mtr['f2']:.1f}", fontsize=10)
        vmax = cm.max()
        for r in range(2):
            for c in range(2):
    ...
    for j in range(len(panels), len(axes)):
        axes[j].axis("off")
    fig.suptitle(f"CSIC models on ECML test — Confusion Matrices "
                 f"({n:,} samples, {n_atk/n*100:.1f}% attack)", fontsize=14, fontweight="bold")
    plt.tight_layout(rect=[0,0,1,0.97])
    plt.savefig(os.path.join(CHARTS_DIR, "csic_on_ecml_confusion.png"), dpi=150, bbox_inches="tight")
    plt.close()

    print(f"[charts] saved csic_on_ecml_metrics.png + csic_on_ecml_confusion.png to {CHARTS_DIR}")
    print("\nDone. CSIC models untouched; results written under datasets/ecml/charts/.")
